...
export const refArchSpecs = {
  "d365-finance-operations-replication": {
    zones: [
      { title: "Source", nodes: [{ id: "src", logo: "d365", label: "D365 F&O", sub: "SalesTable · InventTrans +", viz: "tables" }] },
      { title: "Ingest", nodes: [{ id: "link", logo: "synapse", label: "Synapse Link", sub: "CDC · auto export" }] },
      { title: "Store", nodes: [{ id: "adls", logo: "azure", label: "ADLS Gen2", sub: "timestamped folders", viz: "files" }] },
      { title: "Process", nodes: [{ id: "dbx", logo: "databricks", label: "Auto Loader", sub: "changelog rotation", viz: "fan" }] },
      { title: "Serve", nodes: [{ id: "delta", logo: "deltalake", label: "Delta Lake", sub: "1,500+ tables", viz: "medallion" }] },
    ],
    flows: [
      { from: "src", to: "link", step: 1 },
      { from: "link", to: "adls", step: 2, label: "~15 min" },
      { from: "adls", to: "dbx", step: 3 },
      { from: "dbx", to: "delta", step: 4 },
    ],
    band: { title: "Observability", label: "Application Insights → Azure Monitor alerts · KQL", step: 5 },
    dataflow: [
      "Azure Synapse Link captures change data from D365 Finance & Operations tables.",
      "Exports land in ADLS Gen2 as CSV snapshots in timestamped folders (~15 min cadence) with a changelog and model.json.",
      "A monitor watches the changelog and rotates Auto Loader to the newest folder, keeping the last 5 checkpoints — the old stream is terminated cleanly.",
      "PySpark streams merge into Delta Lake with exactly-once semantics, running 1,500+ tables in parallel.",
      "Application Insights feeds Azure Monitor alerts over KQL for production telemetry — cost tuned from $1,500 to $100/day (93%).",
    ],
  },
